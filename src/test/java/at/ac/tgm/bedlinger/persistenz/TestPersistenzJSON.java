...
public class TestPersistenzJSON {
    ...
    public void setUp() {
        ...
        this.persistenz = new PersistenzJSON();
    }

    @Test
    @DisplayName("Testen der Erstellung eines PersistenzJSON-Objekts")
    public void testPersistenzXML() {
        Persistenz persistenz1 = new PersistenzJSON();
        ...
        Persistenz persistenz2 = new PersistenzJSON(PATH);
    ...
    @Test
    @DisplayName("Testen des Speichern und Laden eines WortTrainers in eine JSON-Datei")
    public void testSaveAndLoadWortTrainer() {
        ...
        try {
            ...
            String fileLocation1 = Paths.get(this.persistenz.getStandardPath(), "worttrainer.json").toString();
            Assertions.assertTrue(new File(fileLocation1).exists());
        ...
        try {
            this.persistenz.save(this.wortTrainer);
            String fileLocation2 = Paths.get(this.persistenz.getStandardPath(), "worttrainer.json").toString();
            Assertions.assertTrue(new File(fileLocation2).exists());
            WortTrainer wortTrainer = this.persistenz.load();
            for (int i = 0; i < this.wortTrainer.getWortliste().size(); i++) {
                Assertions.assertEquals(this.wortTrainer.getWortliste().get(i).getWort(),
                        wortTrainer.getWortliste().get(i).getWort());
